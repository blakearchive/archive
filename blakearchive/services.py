    ...
    def query_objects(cls, config):
        ...
        def object_query(config):
            ...
            if config.get("searchNotes"):
                offset = config.get("objectNotesOffset", 0)
                search_string = generate_search_element("notes", config.get("searchString"))
                solr_results = blake_object_solr.search(search_string, start=offset)
                results["notes"]["results"] = [transform_result(result) for result in solr_results]
                results["notes"]["count"] = solr_results.hits
            return results
        # updated search
